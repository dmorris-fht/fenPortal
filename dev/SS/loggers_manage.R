...
# }
source("../global.R")
source("logger_modal.R")
source("logger_installs_modal.R")
source("logger_configs_modal.R")
...
loggers_manageUI <- fluidPage(
  ...
  column(12,
         column(4,
                ...
                div(
                  style = "margin-top: 10px;",
                  withSpinner(DT::DTOutput(outputId = "loggersTable"), type = 7)
                )
         ),
         column(4,
                ...
                div(
                  div(
                    ...
                    disabled(actionButton(
                      inputId = "remove_logger_install",
                      label = "Remove active logger installation",
                      icon = icon("minus"),
                      class = "btn-success"
                    )),
                    disabled(actionButton(
                      inputId = "add_logger_install",
                ...
                div(
                  style = "margin-top: 10px;",
                  withSpinner(DT::DTOutput(outputId = "logger_installsTable"), type = 7)
                )                
                ),
         column(4,
                h3("Logger configuration record"),
                div(
                  div(id = "add_logger_config_container",
                    style = "margin-top: 10px;",
                    disabled(actionButton(
                      inputId = "add_logger_config",
                ...
                div(
                  style = "margin-top: 10px;",
                  withSpinner(DT::DTOutput(outputId = "logger_configsTable"), type = 7)
                ) 
         )
  ),
...
loggers_manageServer <- function(input, output, session){
  
  con <- fenDb("dmorris","Taraxacum1!") ##DELETE THIS
  role <- "cru"
  ...
  if(!grepl("u",role,fixed = TRUE)){
    ...
    shiny::observeEvent(input$add_logger, {
      logger_modal_dialog(serial = "", model = "", purchase_date = "", firmware = "", hardware = "", comm_config = "", notes = "", edit = FALSE
      )
      rv$add_or_edit <- 1
    })
    ...
    observeEvent(input$add_logger_install,{
      logger_installs_modal_dialog(install = "", install_date = "", install_by = "", install_notes = "", remove_date = "", remove_by = "", remove_notes = "", edit = "add")
    ...
    observeEvent(input$remove_logger_install,{
      ...
      logger_installs_modal_dialog(install = a$install, install_date = a$install_date, install_by = a$install_by, install_notes = a$install_notes, remove_date = "", remove_by = "", remove_notes = "", edit = "remove")
    ...
    observeEvent(input$add_logger_config,{
      logger_configs_modal_dialog(name = "", create_date = "", create_by = "", computer_name = "", application = "", application_version = "", 
    ...
    observeEvent(input$loggersTable_rows_selected,ignoreNULL = FALSE,{
      ...
      if(isTruthy(input$loggersTable_rows_selected)){
        ...
        rv$df_li <- rv$logger_installs[0,]
        rv$df_lc <- rv$log_conf[0,]
    ...
    observeEvent(input$current_id, {
      ...
      if(!is.null(input$current_id) & stringr::str_detect(input$current_id, pattern = "logs_edit")){      
        ...
        df <- rv$df[rv$dt_row, ]
        logger_modal_dialog(serial = df$serial, model = df$model, purchase_date = df$purchase_date, 
      ...
      if(!is.null(input$current_id) & stringr::str_detect(input$current_id, pattern = "log_insts_edit")){      
        ...
        logger_installs_modal_dialog(install = df_li$install, install_date = df_li$install_date, install_by = df_li$install_by, 
      ...
      if(!is.null(input$current_id) & stringr::str_detect(input$current_id, pattern = "log_conf_edit")){      
        ...
        logger_configs_modal_dialog(name = df_lc$name, 
